...
--
--   l1_i.req -> mac_receiver -> vlan_demux -+-> committed_to_axi4_stream -> routed_tx_o(0)
--                                           +-> ...                                  (N-1)
--
-- RX (frames entering through routed_rx_i, leaving towards the peer):
--
--   routed_rx_i(i) -> axi4_stream_to_committed -> vlan_mux -> mac_transmitter -> l1_o.req
--
...
--
-- Port direction convention: l1_i groups everything the peer drives,
-- l1_o everything this entity drives.  See the field mapping on the
-- ports below.
...
entity vlan_path_router is
  ...
  port(
    ...
    --   l1_i.req : frames coming from the peer
    --   l1_i.ack : peer readiness for the frames we send it
    --   l1_o.req : frames going to the peer
    --   l1_o.ack : our readiness for the frames the peer sends us
    l1_i : in  committed_bus_t;
    l1_o : out committed_bus_t;
