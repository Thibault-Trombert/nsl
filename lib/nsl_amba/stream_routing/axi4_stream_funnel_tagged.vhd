...
entity axi4_stream_funnel_tagged is
  generic(
...
architecture beh of axi4_stream_funnel_tagged is
...
begin
  ...
  transition: process(r, in_i, out_i, claim_i)
      variable fair_sel_v : boolean;
  begin
    rin <= r;
    ...
    case r.state is
      ...
      when ST_ELECT =>
        for i in source_count_c - 1 downto 0 loop
          if is_valid(in_config_c, in_i(i))
            ...
            if claim_i(i) = '1' then
              rin.state <= ST_FORWARD_CLAIMED;
            end if;
          end if;
        end loop;
        if not fair_sel_v then
          for i in source_count_c - 1 downto 0 loop
      ...
      when ST_FORWARD =>
        if is_valid(in_config_c, in_i(r.elected)) and is_last(in_config_c, in_i(r.elected)) and is_ready(out_config_c, out_i) then
          rin.state <= ST_ELECT;
          if claim_i(r.elected) = '1' then
            rin.state <= ST_FORWARD_CLAIMED;
          end if;
        end if;

      when ST_FORWARD_CLAIMED =>
        if is_valid(in_config_c, in_i(r.elected)) and is_last(in_config_c, in_i(r.elected)) and is_ready(out_config_c, out_i) then
          rin.state <= ST_ELECT;
          if claim_i(r.elected) = '1' then
            rin.state <= ST_FORWARD_CLAIMED;
        ...
        if not is_valid(in_config_c, in_i(r.elected)) and claim_i(r.elected) = '0' then                                                                                                           
          rin.state <= ST_ELECT;
        end if; 
    end case;
  ...
  mux: process(r, in_i, out_i, claim_i)
  begin
    for i in in_o'range
    loop
      in_o(i) <= out_i;

      if i /= r.elected or (r.state /= ST_FORWARD and r.state /= ST_FORWARD_CLAIMED) then
        in_o(i).ready <= '0';
      end if;

    end loop;

    is_prio_o <= to_logic(claim_i(r.elected) = '1');
    ...
    if r.state /= ST_FORWARD and r.state /= ST_FORWARD_CLAIMED then
      out_o.valid <= '0';
    end if;
