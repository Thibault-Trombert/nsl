...
-- played into l1_i (playing the peer) and checked out on their own
...
-- checked out on l1_o, re-tagged for its own VID with a freshly
-- computed FCS, as vlan_mux/mac_transmitter are documented to
-- produce it.
--
...
architecture beh of tb is
  ...
  -- Stimulus into l1_i: peer's own business whether it pads, mac_receiver
  ...
  -- Expected out of l1_o: mac_transmitter enforces the 64-byte Ethernet
  ...
  -- l1_i/l1_o: split by who drives, not by data direction (see
  -- vlan_path_router.md). l1_in_s carries both the peer's outbound
  -- frames (.req) and the peer's readiness for the dut's outbound
  -- frames (.ack); l1_out_s is the mirror, entirely dut-driven.
  signal l1_in_s, l1_out_s : committed_bus_t;
  ...
  -- committed_to_axi4_stream adapters wrapped around each pipe, so
  -- every side of this dut can be driven/checked with the same
  -- nsl_bnoc.testing vocabulary as l1_i/l1_o.
...
begin
  ...
  begin
    ...
    committed_put(l1_in_s.req, l1_out_s.ack, clock_s,
  ...
  begin
    ...
    committed_check("l1_o vid10", l1_out_s.req, l1_in_s.ack, clock_s,
    ...
    committed_check("l1_o vid20", l1_out_s.req, l1_in_s.ack, clock_s,
  ...
  dut: entity work.vlan_path_router
    generic map(
      vlan_id_c        => vlan_id_c,
